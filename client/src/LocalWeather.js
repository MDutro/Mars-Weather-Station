...
export class LocalWeather extends React.Component {
  ...
  onFormSubmit = (event) => {
    ...
    if (this.state.zip) {
      axios
        .get(`http://localhost:3001/search?zip=${this.state.zip}`)
        ...
        .then(() =>
          this.setState({
            ...
            localMin: this.state.localWeather.forecast.forecastday[0].day
              .mintemp_c,
            localMax: this.state.localWeather.forecast.forecastday[0].day
              .maxtemp_c,
          })
        )
